...
The repository has two branches, a `master` branch, and a `lite` branch. The `master` branch aims to showcase all the supported features (requiring a relatively lot of time and memory to compile), while the `lite` branch supplies a stripped down, light version of the template.
